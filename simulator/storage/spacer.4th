...
: MODE 22 EMIT EMIT ;
: CURSOR 23 EMIT 1 EMIT EMIT 0 EMIT 0 EMIT 0 EMIT 0 EMIT 0 EMIT 0 EMIT 0 EMIT ;
...
139 CONSTANT KEY-UP
138 CONSTANT KEY-DOWN
136 CONSTANT KEY-LEFT
137 CONSTANT KEY-RIGHT
...
VDU SCROLL-SEQ
  ...
  23 C, 7 C, 0 C, 1 C, 0 C,     \ scroll text window 8 pix to the left
  0 C, 0 C, 0 C, 0 C, 0 C,
...
: SCROLL-BACKGROUND
	...
	FUEL @ 1024 < IF
	    ...
	    10 28 AT-XY 63 COL ." Time to refuel in tunnel"
	    8 SCROLL-PERIOD !
        ELSE	    
...
: TYPE-BACKGROUND
    OVER 20 > IF 27 ELSE 63 THEN COL
    >R 2DUP AT-XY
    40 * + PLAYFIELD + R> TYPE
;    
...
: SHOW-SCORE
	...
	1 #SHIPS +!
    THEN
    0 29 AT-XY 63 COL ." SHIPS " #SHIPS @ 3 .R SPACE
    ." FUEL " 9 COL FUEL @ 10 RSHIFT 1+ 0 DO [CHAR] + EMIT LOOP 10 SPACES
    29 29 AT-XY 63 COL ." SCORE" SCORE @ 5 .R
;
...
: COLLISION-DETECT ( --- f)
    ...
    $8080 AND IF
	TRUE
	10 28 AT-XY 63 COL ." Crashed into ground"
	SHOW-EXPL
	EXIT
    THEN 
    LASER-TEMP @ 20 > IF
	TRUE
	10 28 AT-XY 63 COL ." Laser overheated" 
	SHOW-EXPL
	EXIT
    THEN
    SHIP-DATA SHIP-ARRAY-SIZE  BOUNDS BYTES-PER-SHIP + DO
...
: MAINLOOP
	...
	KEY? IF
		...
		KEY-UP OF LIFT @ NEGATE  0 SET-RATE-Y ENDOF
		KEY-DOWN OF LIFT @  0 SET-RATE-Y  ENDOF
		KEY-LEFT OF -8 0 SET-RATE-X ENDOF
		KEY-RIGHT OF 8 0 SET-RATE-X ENDOF
		BL OF 1 LASER-ON ENDOF
		[CHAR] Q OF QUITTING ON 0 #SHIPS ! ENDOF
	    ENDCASE
	THEN
	LASER-TEMP @ IF
	    -1 LASER-TEMP +!
...
: INSTRUCTIONS
    ...
    27 COL 14 0 AT-XY ." S-P-A-C-E-R" 63 COL
    CR
    ...
    CR
    CR
    CR 5 COL 137 EMIT 138 EMIT 63 COL 10 SPACES ." 50 points"
    CR
    CR 6 COL 139 EMIT 140 EMIT 63 COL 10 SPACES ." 60 points"
    CR
